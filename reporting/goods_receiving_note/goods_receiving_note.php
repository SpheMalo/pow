...
if (isset($stock))
{
    $sql = "SELECT product.id as prodID, product.number as prodNo, product.name as prodName, product.description as prodDesc,  
            product.quantity as prodQty, product.size as prodSize, type_product.name as prodType, order.number as orderNo
                FROM `stock`
    ...
    if ($r->rowCount() > 0){
        while($row = $r->fetch()) {
            ...
            $stocked[$c]['prodSize']= $row['prodSize'];
            $stocked[$c]['prodQty']= $row['prodQty'];
            $c++;
        }
        echo json_encode($stocked);
        console.log(json_encode($stocked));
    }
    else {
        echo false;
    }
